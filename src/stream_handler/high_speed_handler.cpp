...
void high_speed_handler(std::string streamloc, std::string output){
    std::ifstream in(streamloc.c_str(), std::ios::binary | std::ios::in);
    std::ofstream out(output.c_str(), std::ios::binary | std::ios::out);

    if(!in.is_open() | !out.is_open()){
        std::cerr<<"[HSH] Cannot open stream location or the output file to write to."<<std::endl;
        return;
    }
    ...
    while(!in.eof()){
        ...
        if(in.fail()){
            std::cout<<"HSH ended stream. Returning"<<std::endl;
            break;
        }

        std::cout<<"HSH "<<(char)blockType[0]<<(char)blockType[1]<<(char)blockType[2]<<(char)blockType[3]<<"\n";
