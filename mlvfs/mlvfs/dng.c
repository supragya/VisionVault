...
//        // TODO: ADD HERE LINEARIZATION TABLE
//        position -= sizeof(uint32_t);
//        position -= sizeof(struct directory_entry) * EXIF_IFD_COUNT;
//        position -= sizeof(uint16_t);
//        *(uint16_t*)(header+position) = EXIF_IFD_COUNT+1;
//        position += sizeof(uint16_t);
//        position += sizeof(struct directory_entry)*EXIF_IFD_COUNT;
//
//        #define steps 2
//        struct directory_entry temp = {tcLinearizationTable, ttShort, steps, 0};
//        memcpy(header+position, &temp, sizeof(struct directory_entry) - sizeof(uint32_t));
//        position += (sizeof(struct directory_entry));
//        uint16_t arr[steps*steps*steps] = {14, 12, 23, 33, 41, 100, 200, 211};
//        memcpy(header+position, arr, steps*steps*steps*sizeof(uint16_t));
//        position += steps*steps*steps*sizeof(uint16_t);
//
//        uint32_t zero = 0;
//        memcpy(header + position, &zero, sizeof(uint32_t));
//        position += sizeof(uint32_t);
//
//        // TODO: ADD HERE LINEARIZATION TABLE
